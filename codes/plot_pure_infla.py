"""
Plot the data assimilation results: pure infla
Read:
  x_t.txt
  x_b.txt
  x_a.txt
...
ax1.set_title(r'RMSE(Pure Inflation; ensemble member:300)', size=20)
ax1.set_ylim(0,5.5)
ax1.set_xlim(0,10)
ax1.grid(color = 'gray', linestyle = '--', linewidth = 1)
ax1.plot(x_new, smooth_noda, 'g-' , label=r'$RMSE$_noDA')
ax1.plot(x_new, smooth_EnKF, 'b-' , label=r'$RMSE$_EnKF(alpha=0.8)')
ax1.plot(x_new, smooth_EnKF01, 'k-' , label=r'$RMSE$_EnKF(alpha=0.85)')
ax1.plot(x_new, smooth_EnKF04, 'm-' , label=r'$RMSE$_EnKF(alpha=0.9)')
ax1.plot(x_new, smooth_EnKF02, 'r-' , label=r'$RMSE$_EnKF(alpha=0.95)')
#ax1.plot(x_new, smooth_EnKF, 'b-' , label=r'$RMSE$_EnKF(alpha=0.8)')
#ax1.plot(x_new, smooth_EnKF04, 'm-' , label=r'$RMSE$_EnKF(alpha=0.9)')
